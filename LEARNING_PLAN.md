...
- [x] **Newly parked (decide in Phase 1) — closed in 1.5:** two picking systems still run side by side, configured through two different mechanisms — `collision_mask`/`exclude` in code for the raycast, `input_ray_pickable` per body in the inspector for physics picking. Only one should survive. Also unresolved: the ray is built in the input frame but cast in the next physics frame, while the camera hangs off a body that moves every physics frame. *Both resolved. The manual raycast with `collision_mask` won, because the collider is only a ray-stopper here — it never decides which hex was hit, so per-body input events buy nothing. And the ray is now built from the current mouse position at the top of `_physics_process` and cast once: `from`/`to` were members holding a value that is a temporary, which is exactly what produced the one-frame lag.*
...
### Step 1.5 — Click a hex **[REVIEW]** ✅ *done 2026-08-14*

- [x] **Goal:** Connect input to the grid — hover highlights a hex, click selects it.
- [x] **Learn first:** World-to-hex conversion and **cube rounding** (Red Blob covers it). This is where naive rounding produces wrong results near hex boundaries. *`round_axial` was already there from 1.2 and needed no changes — the step was really just inverting the layout formula and feeding it through.*
- [x] **Build:** Raycast → world position → hex coordinate → highlight that tile. *Three nodes: `RaycastManager` knows nothing about hexes, `SelectionManager` nothing about meshes, `GridVisualManager` nothing about input. That split is why every bug below was a one-liner rather than a restructure.*
- [x] **Test:** Round-trip tests — hex → world → hex should be identity for every hex on the board. Write these yourself; they're a good first test-writing exercise. *Written across four hex sizes including awkward ones, plus the nearest-centre property over 400 random points per size — the strongest formulation, and it needs no hand-computed expected values at all. Randomised per run with the **seed printed on failure**: new coverage every run, and a failure is still reproducible.*
- [x] **Done when:** Hover highlights the correct hex every time, including right at the boundaries between tiles. *This is the criterion the first attempt failed. Hover emitted the collider's **centre** while the click emitted the ray's **hit point**, so the two answered the same question by different means — and since the round collision cylinder uses the circumradius, neighbouring colliders overlap and the two disagreed exactly at the boundaries. Both now go through the same hit point.*
- [x] **[REVIEW]** First real architecture review. I'll look at whether the logic/view boundary is holding, your static typing discipline, signal usage, and naming. Worth doing properly — corrections here are cheap and compound. *Boundary holds: `logic/` is still nothing but `hex.gd`, and the world mapping stays in `view/` because `hex_size` is a presentation value. Carried forward to 1.6: every tile shares one mesh and one collision shape, which is correct while all tiles are identical and breaks the moment they have individual heights.*

> **One root cause produced four separate bugs in this step: a value type cannot express "absent".** `Vector2i` has no null and its zero value `(0,0)` is a real board coordinate; `Vector3` cannot be `null` at all. So the presence of a highlight has to live in a separate flag — and then *every* check must read the flag, not just the value. The four faces of it: an out-of-board sentinel coordinate so the centre tile could be clicked at all; a `!= old_tile` comparison that silently did nothing for the centre tile; a `position == null` branch that could never be entered, leaving the mouse-off-board case unhandled; and a re-entry check that ignored the flag, so leaving the board and returning to the *same* tile showed no hover. Where value and flag were read together — `is_tile_hovered and tile == hovered_tile` in the render pass — nothing ever went wrong.

> **Second lesson: the fix for the hover/select conflict was structural, not a special case.** Both highlights had been writing the same mesh slot and each restored it to the default, so whichever moved last erased the other. Giving each its own node, and then computing every tile's appearance from the desired state instead of diffing against the previous one, removed the whole class — including two of the bugs above, which had been living in the diff branches. Diffing is an optimisation; at 91 tiles and one change per frame there was nothing to optimise.
