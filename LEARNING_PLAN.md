...
### Step 0.3 — Picking objects in 3D 🔨 *in progress, started 2026-07-29*
...
- [ ] **Build:** Click anywhere on the floor → print the exact world coordinates. *Not done — this is the half that matters for hexes.* Click a cube → identify *which* cube by name. *Done via the raycast; the `_input_event` route also still works.*
- [ ] **Done when:** Clicks report correct positions reliably, including near the screen edges.
- [ ] **Traps:** Forgetting that a body needs a `CollisionShape3D` to be hit at all. Layer/mask confusion is the number one source of "my raycast hits nothing" — when it silently fails, check masks first. *Deliberately left to be learned by breaking rather than reading.*
- [ ] **[EXPLAIN]** Describe how a screen-space click becomes a world-space position. If you can't narrate the whole chain, re-read before moving on.
- [ ] **Open from review:** space state is cached in `_ready()` and queried from `_input` — check the doc note on when it's valid and which callback physics queries belong in. `_input` vs `_unhandled_input` for world clicks under UI. `RayIntersectionResult` field list vs. the actual doc table, `Dictionary.get()` defaults, `_to_string` instead of `to_custom_string()`. Picking currently lives on the `Camera3D` — ownership question, unresolved on purpose.
...
- [ ] **Goal:** Create the actual project with an architecture that won't need unpicking later.
- [ ] **Learn first:** Godot's `.gitignore` requirements (ignore `.godot/`, keep `.import` handling correct for your version). GUT installation — the Godot Unit Test addon, via the Asset Library or a git clone into `addons/`.
- [ ] **Build:** New project. The folder structure from the architecture rule above. `git init` — the project isn't under version control yet and should be from day one. Install GUT and get it running.
- [ ] **Build — strict mode, do this before writing a single line:** turn the GDScript warnings up in `project.godot` (`Project Settings → Advanced → Debug → GDScript`). `untyped_declaration`, `unsafe_property_access`, `unsafe_method_access`, `unsafe_call_argument`, `unsafe_cast`, `shadowed_variable` and `standalone_expression` to **Error** (`=2`); `unused_variable`, `unused_parameter`, `unused_signal`, `return_value_discarded`, `integer_division` to warn (`=1`). Leave `inferred_declaration` off — `:=` is statically typed and idiomatic. Also `Editor Settings → Text Editor → Completion → Add Type Hints` on, and `Trim Trailing Whitespace on Save` on.
- [ ] **Done when:** GUT runs successfully with zero tests, and you have a first commit. Bonus: get GUT running from the command line, not just the editor panel — you'll want that later.
- [ ] **Traps:** Committing `.godot/`. Also, don't over-engineer the folder structure now; four folders is enough.
...
### Step 1.2 — Hex coordinates **[TESTS PROVIDED]**

- [ ] **Goal:** Complete hex math with zero engine dependencies. This is the most important step in Phase 1 — it establishes the logic/view split in the very first real code you write.
- [ ] **Learn first:** Red Blob Games' hex grids page (redblobgames.com/grids/hexagons). Read the coordinate systems section properly and *decide* between axial and cube — don't drift into one. Understand why cube coordinates make distance trivial.
- [ ] **Build:** A `HexCoord` class extending `RefCounted` (or a struct-like approach — form an opinion). Neighbours, distance, line drawing, range/spiral. Fully statically typed.
- [ ] **Test:** I provide the suite. Make it pass.
- [ ] **Done when:** All provided tests green, and `logic/hex/` contains no reference to any Node type whatsoever.
- [ ] **Traps:** Putting pixel/world position on the coordinate class. Coordinates are pure math; the mapping to world space is a *view* concern and belongs in the next step. If you feel tempted, that's the exact instinct this architecture exists to train out.

### Step 1.3 — Rendering the grid
